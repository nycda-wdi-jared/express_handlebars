...
module.exports = (app, passport) => {
	...
	*/
	...
	app.get('/api/footer', function(req,res){
		if(req.user){
			res.json({message: 'signed-in', user: req.user.name});
		}
	});
	...
	*/
	...
	app.get('/api/all-users-posts', (req,res) => {
				...
				for(var i = 0; i < users.length; i++){
					for(var j = 0; j < posts.length; j++){
						if(users[i].id == posts[j].userID){
							postsArr.push({name: users[i].name, message: posts[j].message, createdAt: posts[j].createdAt})
						}
					}
				}
				res.json(postsArr)
